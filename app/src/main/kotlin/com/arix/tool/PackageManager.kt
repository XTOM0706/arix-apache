...
fun createBuiltInPackages(context: Context): List<PackageDef> = listOf(
    ...
    PackageDef("sub_agent", "子 Agent", "AI 创建子 agent 并行处理多个独立子任务，每个子 agent 能自己调工具、跑完汇总。把复杂活拆成可并行的部分。", "Agent",
        listOf(SubAgentTool(context)), enabledByDefault = false),
    PackageDef("workflow", "工作流", "把一串工具调用+变量传递存成可复用流程，一键或让 AI 跑；每步照走权限审批、可被停止。", "Agent",
        listOf(WorkflowTool(context)), enabledByDefault = false),
    ...
    PackageDef("ai_guard", "AI 风控", "AI操作风险等级管控，敏感操作需用户确认。", "系统",
        listOf(AIGuardTool()), enabledByDefault = true),
    // 设备控制 (日用)
    ...
    PackageDef("skill", "技能录制", "录制一串界面操作存成技能，一键回放（免每次现看现点）。需开无障碍服务，免 root。", "工具",
        listOf(SkillTool(context)), enabledByDefault = false),
