 ...
 * 人设 + 相关记忆 + 模型系统提示。让唤醒出来的是「带记忆的他」而非通用 bot。
 *
 * 独立实现、复用现成 manager（MemoryManager / IdentityPrefs / AssistantRolePrefs），
...
object WakeAssistantContext {
    ...
    /**
     * 拼系统提示。
     * @param cardId       已废弃（角色卡体系移除），保留形参避免调用方改动。
     * @param convId       当前会话（临时可为 null）
     * @param userText     本轮用户话（用于检索相关记忆）
     * @param continuation 会话续接摘要（由调用方在开场时算一次传入，避免每轮重算）
     * @param apiSystemPrompt 模型配置里的系统提示
     */
    ...
    ): String? = withContext(Dispatchers.IO) {   // 内含健康(可能读 Gadgetbridge SQLite)/记忆读——别在唤醒的主线程跑，防 ANR
        val app = context.applicationContext
        val mem = runCatching { MemoryManager(app).queryRelevant(userText, 5, cardId) }.getOrDefault(emptyList())
        val characterSetting = AssistantRolePrefs.characterSetting(app)
        buildString {
            ...
            characterSetting.takeIf { it.isNotBlank() }?.let { append(it.trim()).append("\n\n") }
