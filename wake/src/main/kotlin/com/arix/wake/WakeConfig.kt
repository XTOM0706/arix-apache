...
/**
 * L2 判决器选择。见 DESIGN-WAKE.md §5。
 * 目前只有自定义唤醒词一条路（KWS tflite 路径从未落地，已删）。
 */
 ...
 * 默认值 = 省电窗口 + 自定义唤醒词判决器。阈值/时长为工程初值，P7 需目标表实测调优。
 */
data class WakeConfig(
    val powerPolicy: WakePowerPolicy = WakePowerPolicy.WINDOWED,
    val detectorMode: WakeDetectorMode = WakeDetectorMode.CUSTOM_ENROLLMENT,
